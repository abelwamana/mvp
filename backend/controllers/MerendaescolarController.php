...
use backend\models\MerendaescolarSearch;
...
/**
 * MerendaescolarController implements the CRUD actions for Merendaescolar model.
 */
class MerendaescolarController extends Controller
{
    /**
     * @inheritDoc
     */
    ...
    /**
     * Lists all Merendaescolar models.
     *
     * @return string
     */
    public function actionIndex()
    {
        $searchModel = new MerendaescolarSearch();
    ...
    /**
     * Displays a single Merendaescolar model.
     * @param int $Id ID
     * @return string
    ...
    /**
     * Creates a new Merendaescolar model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return string|\yii\web\Response
     */
   public function actionCreate() {
        $models = new Merendaescolar();
            ...
            try {
                ...
                //adicionar a data de criaçao
                 if ($models->load($postData)) {
                    ...
                    $file = UploadedFile::getInstance($models, 'anexoTermoEntregaMerendaEscolar');

                    if (!empty($file)) {
                        // Salve o arquivo de upload
                        $folderName = 'anexoTermoEntregaMerendaEscolar'; // Use o nome do atributo como nome da pasta
                        ...
                        if ($file->saveAs($path)) {
                            $models->anexoTermoEntregaMerendaEscolar = $folderName . '/' . $newFileName;
                        } else {
                            \Yii::error("Erro ao fazer upload de anexoTermoEntregaMerendaEscolar");
                        }
                    }
    ...
    /**
     * Finds the Merendaescolar model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param int $Id ID
     * @return Merendaescolar the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($Id)
    {
        if (($model = Merendaescolar::findOne(['Id' => $Id])) !== null) {
            return $model;
        }
    ...
    public static function getEnumValues($attribute) {
        $values = \Yii::$app->db->createCommand("SHOW COLUMNS FROM {{%rastreio}} LIKE '{$attribute}'")->queryOne();
