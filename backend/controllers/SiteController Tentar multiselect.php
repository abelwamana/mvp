...
// Percorra os resultados da consulta e crie o array manualmente
    ...
    public function actionGetMunicipios($ids) {
        // Converter a string de IDs em um array
        $provinciaIds = explode(',', $ids);

        // Consultar todos os municípios cujas províncias estejam no array
        $municipios = Municipio::find()
                ->where(['in', 'provinciaID', $provinciaIds])
                ->all();
    ...
    public function actionGetComunas($ids) {
        // Converter a string de IDs em um array
        $municipioIds = explode(',', $ids);

        // Consultar todas as comunas cujos municípios estejam no array
        $comunas = Comuna::find()->where(['in', 'municipioID', $municipioIds])->all();
    ...
    public function actionAddEvents() {
        ...
        if (Yii::$app->request->post('Event')) {
            ...
            if ($eventData !== null) {
                ...
                $calendarEvento->provincia = is_array($calendarEvento->provincia) ? implode(',', $eventModel->provincia) : $eventModel->provincia;
                $calendarEvento->municipio = is_array($calendarEvento->municipio) ? implode(',', $eventModel->municipio) : $eventModel->municipio;
                $calendarEvento->comuna = is_array($calendarEvento->comuna) ? implode(',', $eventModel->comuna) : $eventModel->comuna;
