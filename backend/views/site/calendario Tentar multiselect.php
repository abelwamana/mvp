            ...
            <div class="row align-items-center" style="margin-left: 1px; max-width: 99.7%;">    
                <div class="col-md-4">
                    ...
                    Select2::widget([
                        ...
                        'pluginOptions' => ['width' => '52%'],
                    ]);
                    ?>
                </div>

                <div class="col-md-4" style="margin-left: -196px;">
                    <?=
                    Select2::widget([
                        'name' => 'municipios',
                        'id' => 'municipios',
                        'value' => '',
                        'data' => $municipios,
                        'options' => ['multiple' => true, 'placeholder' => 'Selecione o(s) Município(s)'],
                        'pluginOptions' => ['width' => '52%'],
                    ]);
                    ?>
                </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <?=
                $form->field($eventModel, 'provincia')->widget(Select2::classname(), [
                    'data' => $provinciasList,
                    'options' => ['id' => 'provincia-select', 'placeholder' => 'Selecione as províncias...', 'multiple' => true],
                    'pluginOptions' => [
                        'allowClear' => true,
                    ],
                ])
                ...
                <?=
                $form->field($eventModel, 'municipio')->widget(Select2::classname(), [
                    'data' => [], // Inicialmente vazio, será preenchido via AJAX
                    'options' => ['id' => 'municipio-select', 'placeholder' => 'Selecione os municípios...', 'multiple' => true],
                    'pluginOptions' => [
                        'allowClear' => true,
                    ],
                ])
                ...
                <?=
                $form->field($eventModel, 'comuna')->widget(Select2::classname(), [
                    'data' => [], // Inicialmente vazio, será preenchido via AJAX
                    'options' => ['id' => 'comuna-select', 'placeholder' => 'Selecione as comunas...', 'multiple' => true],
                    'pluginOptions' => [
                        'allowClear' => true,
                    ],
                ])
...
$(document).ready(function(){
    ...
    var provinciaIDs = $(this).val(); // Aqui, já obtemos um array de IDs
    ...
    if(provinciaIDs.length > 0){
        $.ajax({
            ...
            data: {
               ids: provinciaIDs.join(',') // Converter o array para uma string separada por vírgulas
            },
            dataType: 'json',
            success: function(data){
                $('#municipio-select').empty().trigger('change');
                // Adicionar as opções retornadas no Select2
                $.each(data, function(index, municipio) {
                    var newOption = new Option(municipio.nome, municipio.id, false, false);
                    $('#municipio-select').append(newOption).trigger('change');
                });
                $('#comuna-select').empty().trigger('change');
            },
            error: function() {
...
$('#municipio-select').change(function(){
    var municipioIDs = $(this).val(); // Aqui, já obtemos um array de IDs
    ...
    if(municipioIDs.length > 0){
        $.ajax({
            ...
            data: {
               ids: municipioIDs.join(',') // Converter o array para uma string separada por vírgulas
            },
            dataType: 'json',
            success: function(data){
                $('#comuna-select').empty().trigger('change');
                ...
                $.each(data, function(index, comuna) {
                    var newOption = new Option(comuna.nome, comuna.id, false, false);
                    $('#comuna-select').append(newOption).trigger('change');
                });
            },
            error: function() {
